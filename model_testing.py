...
def model_testing3(model, iterations, node, level):
    ...
    for i in range(0, iterations):
        ...
        result_mid.to_pickle(f'{node}_{level}_testing_mid.p')
        result_low.to_pickle(f'{node}_{level}_testing_low.p')
    observations_df.to_csv(f'{node}_{level}_observations_example.csv')
...
if __name__ == '__main__':
    ...
    model_testing3(commitment_model, 5, "Commitment", 0)
    model_testing3(commitment_model, 5, "Commitment", 1)
    model_testing3(commitment_model, 5, "Commitment", 2)
